...
{
	FREE_DATA_CHUNK,
	FREE_FULL_COUNT_CHUNK,
	FREE_INIT_DATA_CHUNK,
	ALLOC_FULL_INDEX_CHUNK,
	ALLOC_DATA_CHUNK,
	ALLOC_ENTRY_MEMTLS,  
	ALLOC_TLS_FIRST_MEMTLS,         
	ALLOC_ALLOCINDEXFULL_MEMTLS ,
	ALLOC_DATA_MEMTLS, 
	FREE_ENTRY_MEMTLS, 
	FREE_FULL_COUNT_MEMTLS
};

struct MemoryPoolTLS_Log
{
	int64_t _No;
	eMemoryPoolTLS _Pos;
	DWORD _ThreadID;
	...
	void DataSettiong(int64_t no, eMemoryPoolTLS pos, DWORD threadid, int64_t chunkPtr, int32_t allocIndex, LONG  freeCount, bool bFree, int64_t data)
	{
		_No = no;
		_Pos = pos;
		_ThreadID = threadid;
		_ChunkPtr = chunkPtr;
		_AllocIndex = allocIndex;
...
struct IOCP_Log
{
	...
	int64_t _SendOL;
	LONG _SendFlag;
	int64_t _AllocPacket;
	int32_t _SmartRefCount;


	void DataSettiong(int64_t no, eIOCP_LINE pos, DWORD threadid, int64_t socket, DWORD ioCount, int64_t curSession, int64_t  recvOL, int64_t  sendOL, LONG snedFlag,int64_t allocPacket=-1,int32_t smartRefCount=-1)
	{
		_No = no;
		_Pos = pos;
		_ThreadID = threadid;
		...
		_SendOL = sendOL;
		_SendFlag = snedFlag;
		_AllocPacket = allocPacket;
		_SmartRefCount = smartRefCount;
	}
